...
/**
-- Temporal
insert into KafkaToWrite select s1.sec_code, sum(s1.order_vol) as score from MyKafkaSrc01 s1 left join MyDimTable FOR SYSTEM_TIME AS OF s1.ts_timestamp AS s2 on s1.sec_code = s2.channel 
group by TUMBLE(s1.ts_timestamp, INTERVAL '1' SECONDS), s1.sec_code;


-- Interval
insert into KafkaToWrite select s1.sec_code, sum(s1.order_vol) as score from MyKafkaSrc01 s1 left join MyKafkaSrc02 s2 on s1.sec_code = s2.channel 
and s1.proctime BETWEEN s2.proctime - INTERVAL '1' MINUTE AND s2.proctime 
